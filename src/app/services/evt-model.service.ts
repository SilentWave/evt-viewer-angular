...
export class EVTModelService {
    ...
    public readonly objects$ = this.parsedLists$.pipe(
        map(({ lists, entities }) => this.namedEntitiesParser.getResultsByType(lists, entities, ['object'])),
    );
    ...
    public readonly namedEntities$: Observable<NamedEntities> = combineLatest([
        ...
        map(([persons, places, organizations, relations, events, objects]) => ({
            all: {
                lists: [...persons.lists, ...places.lists, ...organizations.lists, ...events.lists, ...objects.lists],
                entities: [...persons.entities, ...places.entities, ...organizations.entities, ...events.entities, ...objects.entities],
            },
            persons,
            places,
            organizations,
